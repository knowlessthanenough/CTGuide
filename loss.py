...
def bmc_loss_md(pred, target, noise_var):
    I = torch.eye(pred.shape[-1],device="cuda").to(torch.float64)
    ...
    loss = loss * (2 * noise_var).detach() #    return torch._C._nn.cross_entropy_loss(input, target, weight, _Reduction.get_enum(reduction), ignore_index)      RuntimeError: Expected target size [16, 200], got [16]
    return loss
