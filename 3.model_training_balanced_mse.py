...
parser.add_argument('--clip', type=float, default=0.1,
                    help='gradient clip, -1 means no clip (default: 0.1)')
parser.add_argument('--epochs', type=int, default=100,
                    help='upper epoch limit (default: 100)')
parser.add_argument('--ksize', type=int, default=5,
                    help='kernel size (default: 5)')
parser.add_argument('--levels', type=int, default=5,
                    help='# of levels (default: 6)') #log base 2((200-1)/(5-1)) = 5.64
...
def train(train_dataset, ep):
    ...
    for i,(one_batch_train_data) in enumerate(train_dataset):
        ...
        loss.backward()
        optimizer.step()
    train_loss = total_batch_loss / count  # epoch loss
    print("Epoch {:2d} | lr {:.5f} | loss {:.5f}".format(ep, lr, train_loss))
    return train_loss
...
def draw_result(lst_loss: list, title: str):
    lst_iter = range(len(lst_loss))
...
if __name__ == "__main__":
    ...
    # criterion = BMCLossMD(init_noise_sigma=1.)
    lr = args.lr
    optimizer = getattr(optim, args.optim)(model.parameters(), lr=lr)
    # optimizer.add_param_group({'params': criterion.parameters(), 'lr': 0.01})  # make the noise learnable
    ...
    for ep in range(1, args.epochs+1):
        train_loss = train(train_data_loader,ep)
        vloss = evaluate(val_data_loader, name='Validation')
        tloss = evaluate(test_data_loader, name='Test')
        if vloss < best_vloss:
            model_name = "para\\breath_{0}.pt".format(ep)
            with open(model_name, "wb") as f:
                torch.save(model.state_dict(), f)
                print("Saved model!\n")
            best_vloss = vloss
        if ep > 20 and vloss > max(vloss_list[-5:]):
            lr /= 10
    ...
    print('-' * 89)
    draw_result(trainloss_list, 'train_loss')
    draw_result(vloss_list, 'val_loss')
    draw_result(tloss_list, 'test_loss')
